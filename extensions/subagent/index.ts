...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    if (!check.exists) ctx.ui.notify("No .pi/MODELS.md found. Run /models setup to draft model routes.", "warning");
    else if (!check.ok) ctx.ui.notify(`Invalid .pi/MODELS.md:\n${check.errors.join("\n")}`, "error");
  });
...
function createSubagentTool(pi: ExtensionAPI): ToolDefinition<typeof SubagentParams, ToolDetails> {
  return {
    name: "subagent",
    label: "Subagent",
    description: "Spawn child Pi sessions, run parallel child jobs, or follow up with an existing child session via sessionFile.",
    ...
    async execute(_id, params: SubagentParamsType, signal, onUpdate, ctx) {
      ...
      if (fanout) {
        onUpdate?.({ content: [{ type: "text", text: `Running ${fanout.length} subagents in parallel. Results will be returned when all finish.` }] });
        const runs = await runParallelSubagents({ tasks: fanout, model: options.model, reasoning: options.reasoning }, ctx, signal);
        return { content: [{ type: "text", text: formatParallelRuns(runs) }], isError: runs.some((run) => run.status !== "complete"), details: { runs } };
      }
      // `sessionFile` means continue an existing child instead of spawning a related duplicate.
      if (params.sessionFile?.trim()) {
        if (!params.task?.trim()) return errorResult("Provide `task` with `sessionFile` for a subagent follow-up.");
        const run = await messageSubagentSession({ sessionFile: params.sessionFile, message: params.task, model: options.model, reasoning: options.reasoning }, ctx, signal);
        return { content: [{ type: "text", text: formatRun(run) }], isError: run.status !== "complete", details: { runs: [run] } };
      }
      if (!params.task?.trim()) return errorResult("Provide `task` for one subagent, `tasks` for parallel subagents, or `task` + `sessionFile` for a follow-up.");
      ...
      pi.events.emit("subagent:complete", { id: run.id, agent: "subagent", success: run.status === "complete", summary: run.output ?? run.error ?? "", exitCode: run.exitCode ?? 1, timestamp: Date.now(), sessionFile: run.sessionFile });
      return { content: [{ type: "text", text: formatRun(run) }], isError: run.status !== "complete", details: { runs: [run] } };
    },
    renderCall(args, theme) {
      const action = args.tasks?.length ? `${args.tasks.length} parallel jobs` : args.sessionFile ? `follow-up: ${args.task ?? ""}` : args.task ?? "";
      return new Text(`${theme.fg("toolTitle", theme.bold("subagent"))}: ${theme.fg("accent", shortTask(action))}`, 0, 0);
    },
