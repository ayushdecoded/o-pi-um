...
export function parseModelsMd(cwd: string): Array<ModelRoute & { name: string; body?: string }> {
  ...
  if (!raw) return [];
  return raw.split(/^##\s+/m).slice(1).flatMap((section): Array<ModelRoute & { name: string; body?: string }> => {
    const [titleLine = "", ...rest] = section.split("\n");
    const body = rest.join("\n");
    const model = body.match(/^model:\s*(\S+)\s*$/m)?.[1];
    const reasoning = body.match(/^(?:thinking|reasoning):\s*(off|minimal|low|medium|high|xhigh)\s*$/m)?.[1] as ThinkingLevelType | undefined;
    const secondaryModel = body.match(/^(?:secondary_model|secondary):\s*(\S+)\s*$/m)?.[1];
    const secondaryReasoning = body.match(/^(?:secondary_thinking|secondary_reasoning):\s*(off|minimal|low|medium|high|xhigh)\s*$/m)?.[1] as ThinkingLevelType | undefined;
    return model ? [{ name: titleLine.trim(), model, ...(reasoning ? { reasoning } : {}), ...(secondaryModel ? { secondaryModel } : {}), ...(secondaryReasoning ? { secondaryReasoning } : {}), body }] : [];
  });
}
...
export function validateModelsMd(cwd: string): { exists: boolean; ok: boolean; errors: string[] } {
  ...
  for (const rule of rules) {
    if (!(REQUIRED_MODEL_SECTIONS as readonly string[]).includes(rule.name)) errors.push(`Unknown section: ${rule.name}`);
    if (!rule.model) errors.push(`Missing model: ${rule.name}`);
    if (!rule.reasoning) errors.push(`Missing thinking: ${rule.name}`);
    else if (!THINKING_VALUES.has(rule.reasoning)) errors.push(`Invalid thinking for ${rule.name}: ${rule.reasoning}`);
  }
...
export function resolveModelRoute(ctx: ExtensionContext, model?: string, reasoning?: ThinkingLevelType): { model?: string; reasoning?: ThinkingLevelType } {
  ...
  const route = parseModelsMd(ctx.cwd).find((item) => item.name.toLowerCase() === model.toLowerCase());
  if (route) {
    ...
    const secondary = route.secondaryModel ? resolveExactAvailableModel(ctx, route.secondaryModel) : undefined;
    if (secondary) return { model: secondary, reasoning: reasoning ?? route.secondaryReasoning ?? route.reasoning };
...
function resolveExactAvailableModel(ctx: ExtensionContext, query: string): string | undefined {
  ...
  return ctx.modelRegistry.getAvailable().map((m) => `${m.provider}/${m.id}`).find((full) => full.toLowerCase() === key);
}
...
function resolveAvailableModel(ctx: ExtensionContext, query: string): string | undefined {
  ...
  const tokens = query.toLowerCase().replace(/['’]s\b/g, "").replace(/[^a-z0-9.]+/g, " ").split(/\s+/).filter((token) => token && !["latest", "newest", "model", "use", "the"].includes(token));
  if (tokens.length === 0) return undefined;
  const candidates = ctx.modelRegistry.getAvailable().map((m) => ({ full: `${m.provider}/${m.id}`, haystack: `${m.provider} ${m.id}`.toLowerCase(), id: m.id })).filter((m) => tokens.every((token) => m.haystack.includes(token)));
...
function compareModelFreshness(a: string, b: string): number {
  ...
  const rank = (id: string) => (id.includes("precision") ? 3 : id.includes("pro") ? 2 : id.includes("flash") ? 0 : 1);
  return rank(a) - rank(b);
}

function modelVersionParts(id: string): number[] {
  const match = id.match(/(?:v|gpt-|glm-|k2\.|qwen3\.)?(\d+(?:[.-]\d+)*)/i);
  return match ? match[1]!.split(/[.-]/).map((n) => Number.parseInt(n, 10)).filter(Number.isFinite) : [];
}
