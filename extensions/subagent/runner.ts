...
import { ensureSessionRoot, makeRunId, normalizeSessionFile, sessionFileForRun } from "./primitives/session.ts";
...
export async function runPiSubagent(params: { task: string; model?: string; reasoning?: ThinkingLevelType }, ctx: ExtensionContext, signal?: AbortSignal): Promise<RunDetails> {
  ...
  return withTrackedRun({ id, task: params.task, model: params.model, startedAt }, ctx, async () => {
    ensureSessionRoot();
    return runPiInTmux({ id, task: params.task, model: params.model, reasoning: params.reasoning, sessionFile, startedAt, cwd: ctx.cwd, depth, freshSessionDir: true, signal });
  });
}
...
export async function messageSubagentSession(params: FollowupParamsType, ctx: ExtensionContext, signal?: AbortSignal): Promise<RunDetails> {
  ...
  const startedAt = Date.now();
  if (!fs.existsSync(sessionFile)) return failedRun(params.message, model, sessionFile, `Session file not found: ${sessionFile}`, startedAt);
  const id = makeRunId("msg");
  return withTrackedRun({ id, task: params.message, model, startedAt }, ctx, async () => {
    return runPiInTmux({ id, task: params.message, model, reasoning, sessionFile, startedAt, cwd: ctx.cwd, depth: currentDepth(), freshSessionDir: false, signal });
  });
}
...
function failedRun(task: string, model: string | undefined, sessionFile: string, error: string, startedAt: number): RunDetails {
  return { id: makeRunId("msg"), task, model, status: "failed", error, sessionFile, startedAt, completedAt: Date.now() };
}
