...
- command context: retained per session with generation/shutdown guards so normal post-turn auto-runs can still use `waitForIdle()` and `navigateTree()`
- current-slice tasks: `name`, `objective`, `verification`, required completion `evidence`, capped at 7
