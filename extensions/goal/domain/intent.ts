...
export function parseGoalIntent(raw: string): ParsedGoalIntent {
  ...
  if (!intent)
    return {
      ...
      turnBudget,
      costBudgetUsd,
      error: "Goal intent must not be empty.",
    };
  if (tokenBudget !== null && (!Number.isSafeInteger(tokenBudget) || tokenBudget <= 0))
    return {
      ...
      turnBudget,
      costBudgetUsd,
      error: "Goal token budget must be a positive integer.",
    };
  if (timeBudgetSeconds.value !== null && timeBudgetSeconds.value <= 0)
    return {
      intent,
      tokenBudget,
      timeBudgetSeconds: timeBudgetSeconds.value,
      turnBudget,
      costBudgetUsd,
      error: "Goal time budget must be positive.",
    };
  if (turnBudget !== null && (!Number.isSafeInteger(turnBudget) || turnBudget <= 0))
    return {
      intent,
      tokenBudget,
      timeBudgetSeconds: timeBudgetSeconds.value,
      turnBudget,
      costBudgetUsd,
      error: "Goal turn budget must be a positive integer.",
    };
  if (costBudgetUsd !== null && (!Number.isFinite(costBudgetUsd) || costBudgetUsd <= 0))
    return {
      intent,
      tokenBudget,
      timeBudgetSeconds: timeBudgetSeconds.value,
      turnBudget,
      costBudgetUsd,
      error: "Goal cost budget must be positive.",
    };
  return {
    intent,
    tokenBudget,
