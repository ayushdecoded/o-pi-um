...
export function goalBudgetPressure(
  goal: GoalState,
): { kind: string; ratio: number; final: boolean } | null {
  const candidates: Array<{ kind: string; ratio: number; final: boolean }> = [];
  if (goal.tokenBudget !== null)
    candidates.push({
      kind: "token",
      ratio: safeRatio(goal.tokensUsed, goal.tokenBudget),
      final: goal.tokensUsed >= goal.tokenBudget,
    });
  if (goal.timeBudgetSeconds != null)
    candidates.push({
      kind: "time",
      ratio: safeRatio(goal.timeUsedSeconds, goal.timeBudgetSeconds),
      final: goal.timeUsedSeconds >= goal.timeBudgetSeconds,
    });
  if (goal.turnBudget != null) {
    const turnsUsed = goal.turnsUsed ?? 0;
    candidates.push({
      kind: "turn",
      ratio: safeRatio(turnsUsed + 1, goal.turnBudget),
      final: turnsUsed + 1 >= goal.turnBudget,
    });
  }
  if (goal.costBudgetUsd != null)
    candidates.push({
      kind: "cost",
      ratio: safeRatio(goal.costUsedUsd ?? 0, goal.costBudgetUsd),
      final: (goal.costUsedUsd ?? 0) >= goal.costBudgetUsd,
    });
  const relevant = candidates.filter((item) => item.final || item.ratio >= 0.7);
...
function formatSubtasksForPrompt(goal: GoalState): string {
  ...
  return subtasks
    .map((item) => `- ${item.completed ? "[x]" : "[ ]"} ${escapeXml(item.title)}`)
    .join("\n");
}
