...
export const GoalToolParamsSchema = Type.Object(
  {
    action: Type.Optional(
      StringEnum(
        ["complete", "subtask", "expand", "pause", "continue"] as const,
        {
    ...
    expansions: Type.Optional(
      Type.Object(
        {
          add: Type.Optional(
            Type.Array(Type.String({ description: "Add objective." })),
          ),
          drop: Type.Optional(
            Type.Number({ description: "Drop objective index." }),
          ),
        },
        { additionalProperties: false },
      ),
      { description: "For expand action." },
    ),
  },
  { additionalProperties: false },
);
...
export function normalizeBoolean(value: unknown): boolean | undefined {
  ...
  if (typeof value === "string") {
    if (/^(true|yes|1|done|complete|completed)$/i.test(value.trim()))
      return true;
